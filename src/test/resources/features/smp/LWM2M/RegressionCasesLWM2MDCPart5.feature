...
Scenario: MB_Sanity_DC_Upgrade-004-Adding test subscriptions for Lwm2m test cases 
	...
	Then user add the below test sets 
		...
		|VZW-Chapter05Cases|  
		|VZW-Ch11-FOTA|
		|VZW-Ch05-UAT-Connectivity-Monitoring-Test|
...
Scenario: VZW-Chapter05Cases
    ...
    Then user switch back to parent window 
    And user clicks on "VZW-Chapter05Cases" test set 
    Then user hits the action "writePartialUpdateResourceOn_4_0_30000_2node" from the DM Impact
	Then user clicks on the "AutomatedPlayButton"
	Then user is navigated to the "VZW-Chapter05Cases" testSet Page 
	Then user select the "_5_07_CLASS_3_APN" folders for test case "5.07 Class 3 APN" 
	Then user selects the "Play" button
	Then user wait for "2" minutes
	Then user validates the "5.07 Class 3 APN" test results for "VZ_TC_LWM2MOTADM" test case 
	Then user hits the action "writePartialUpdateResourceOn_4_0_30000_2node" from the DM Impact
	Then user select the "_5_08_CLASS_6_APN" folders for test case "5.08 Class 6 APN" 
	Then user selects the "Play" button
	Then user wait for "2" minutes
	Then user validates the "5.08 Class 6 APN" test results for "VZ_TC_LWM2MOTADM" test case
	
@MotiveBridge019
Scenario:  VZW-Ch05-UAT-Connectivity-Monitoring-Test :5.01 Network Bearer
     
     Given user closes the current window   
    Then user switch back to parent window 
     And user clicks on "VZW-Ch05-UAT-Connectivity-Monitoring-Test" test set
     Then user selects the "PlayButton" button
     Then user is navigated to the "VZW-Ch05-UAT-Connectivity-Monitoring-Test" testSet Page
     Then user select the "_5_01_Network_Bearer" folders for test case "5.01 Network Bearer"
     Then user selects the "Play" button
     ...
     Then user selects the "Submit" button
     Then user wait for some time
     Then user validates the "5.01 Network Bearer" test results for "VZ_TC_LWM2MOTADM" test case
     Then user closes the simulator in port "5545"
  
	
#@MotiveBridge019
#Scenario: VZW-Ch05-UAT-Connectivity-Monitoring-Test : CHECK
#
#   Given user closes the current window   
#    
#	Then user switch back to parent window 
...
#	Then user hits the action "writePartialUpdateResourceOn_4_0_30000_2node" from the DM Impact
#	Then user is navigated to the "VZW-Ch05-UAT-Connectivity-Monitoring-Test" testSet Page 	
#	Then user select the "_5_10_APN_Persistency_Throughout_Device_Reboot" folders for test case "5.10 APN Persistency Throughout Device Reboot" 	
#	Then user selects the "Play" button 
...
#	Then user hits the action "writePartialUpdateResourceOn_4_0_30000test" from the DM Impact
#	Then user wait for "2" minutes
#	Then user validates the "5.10 APN Persistency Throughout Device Reboot" test results for "VZ_TC_LWM2MOTADM" test case
...
Scenario: VZW-Ch11-FOTA-WIFI-ON-Sufficient_power_to_carry_out_update_OB_11_06_Wifi_On
	...
	Then user wait for "5" seconds
     ...
     Then user verifies the job details for "Insufficient_Battery"
      Then user scroll to the right to "Submit" button
     Then user selects the "Submit" button
     Then user wait for some time
     Then user validates the "11.06 Insufficient Power to carry out the Update OB Wifi On" test results for "VZ_TC_LWM2MOTADM" test case  
	 Then user closes the simulator in port "5546" 
